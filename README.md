...
- 2/ Then simply use that cli image for building for running any make command in Makefile for example: `docker run --rm -v $(pwd)/pgrx-wasi:/pgrx-wasi build-test build_ext_host`