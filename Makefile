...
.PHONY: all install_pgrx install_pgrx clean

all: install install_pgrx init_pgrx


install: ## Step 1: Install cargo-local-install
...
build_ext_host: ## build extension host
	cd $(RUST_WORKSPACE_DIR) && cargo +nightly build --verbose --target wasm32-unknown-emscripten --lib --manifest-path pgrx-examples/world/Cargo.toml


build_ext: build_ext_host
	@echo TODO BUILD THE EXTENSION


dump_wasm: ## dump the wasm output of helloworld
	/opt/python-wasm-sdk/wasisdk/bin/wasm-objdump -x $(CURRENT_DIR)/pgrx-examples/hello-world/target/wasm32-unknown-unknown/debug/helloworld.wasm